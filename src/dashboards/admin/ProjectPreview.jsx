...
const ProjectPreview = ({ project }) => {
    ...
    const startEditConfiguration = useImportStore((state) => state.startEditConfiguration);

    const handleEditClick = () => {
        if (project) {
            startEditConfiguration(project);
        }
    };
        ...
        <div className="p-4 h-full flex flex-col">
            {project ? (
                <div className="flex flex-col gap-6 h-full">
                    ...
                    {/* --- NEW: Configuration Status and Edit Button --- */}
                    <div className="mt-4 pt-4 border-t border-gray-200">
                        {project.IsConfigured ? (
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={handleEditClick} className="flex items-center gap-2 px-3 py-1 text-sm bg-gray-200 hover:bg-gray-300 rounded-md">
                                    <FaEdit /> Edit
                                </button>
                            </div>
                        ) : (
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={handleEditClick} className="flex items-center gap-2 px-3 py-1 text-sm bg-blue-500 text-white hover:bg-blue-600 rounded-md">
                                    Configure Now
                                </button>
                            </div>
                        )}
